...
\begin{table}[tbp]
	...
	\resizebox{0.55\linewidth}{!}{
		\begin{tabular}{|c|l|}
			...
			\hline
			$r$ & predicted result of a component\\
			\hline
			$y_S'$ & output of SGD\\
			\hline
			$R$ & predicted result of RL-BRT\\
			\hline
			$y'$& output of RL-BRT\\
			\hline
...
\end{equation}
where $\mu_\theta$ is the deterministic policy function with parameter $\theta$. $Q^\mu(s, a)$ is the value function on $\mu$. For a given $s$, the policy function generates an $a$, which is denoted as $a=\mu_\theta(s)$. $J(\cdot)$ is the performance objective.
